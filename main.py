...
async def select_subject(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Ushbu fan bo'yicha maktabdagi <b>jami dars soatini</b> kiriting (Faqat raqam, masalan: 45):",
        parse_mode="HTML"
    )
...
async def process_total_hours(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("Iltimos, faqat raqam kiriting (Masalan: 45):", reply_markup=main_kb)
        return
...
# O'zboshimcha matn yoki rasm yuborilganda xatolik chiqarmaslik va tartibga chaqirish uchun filtr
@dp.message()
