...
async def calculate_and_show(message: types.Message, state: FSMContext, data: dict, teachers: list):
    ...
    # 3-bosqich: Agar shunda ham o'qituvchilar yetishmay soat ortib qolayotgan bo'lsa, 
    # mavjud o'qituvchilarning 1.5 stavka limitiga qat'iy amal qilgan holda qoldiqni iloji boricha taqsimlash
    # (Agar barcha o'qituvchilar 1.5 stavkada bo'lsa va yana ortsa, so'nggi chora sifatida xabar beriladi)
    ...
    if remaining_hours > 0:
        res_text += f"\n⚠️ <b>Diqqat:</b> Barcha o'qituvchilar 1.5 stavka (maksimal {max_limit} soat) limitiga yetdi. Taqsimlanmay qolgan soat: <b>{remaining_hours} soat</b> (Yangi o'qituvchi qo'shish talab etiladi)."
    else:
